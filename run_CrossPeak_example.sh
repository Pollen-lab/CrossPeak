...
base_folder=$(pwd)         # Base folder where the R scripts are located
current_folder="./Example_dataset" # Folder to save all intermediate results and logs
output_folder="./Example_dataset/Output" # Folder to save results
HAL_FILE="${current_folder}/primates.hal" # HAL file with species' genomes and ancestral genome
...
if [[ ! -d "$current_folder" ]]; then
  ...
  echo "Created output folder: $current_folder"
fi
...
ls "$current_folder" > "$initial_files"

# Step 1: Run R script to generate initial data
echo "Running Step 1 in R..."
Rscript "${base_folder}/CrossPeak_Step1.R" > "${current_folder}/step1.log" 2>&1
if [[ $? -ne 0 ]]; then
  echo "Error: Step 1 failed. Check ${current_folder}/step1.log for details."
  exit 1
fi

# Step A: Run halLiftover for each species
echo "Running Step A: halLiftover..."
hal_liftover_log="${current_folder}/stepA_halLiftover.log"
for sp in "${species[@]}"; do
  input_file="${current_folder}/${sp}_summits.bed"
  output_file="${current_folder}/${sp}_summits_lo.bed"
  ...
  if [[ "$use_docker" == true ]]; then
    docker run --rm -v "$base_folder:$base_folder" -w "$base_folder" "$docker_image" halLiftover "$HAL_FILE" "${sp^}" "$input_file" "$ANCESTRAL_GENOME" "$output_file"  >> "$hal_liftover_log" 2>&1
  else
...
hal_liftover_log="${current_folder}/stepB_halLiftover.log"
consensus_input_file="${current_folder}/consensus_summits.bed"
...
for sp in "${species[@]}"; do
  consensus_output_file="${current_folder}/${sp}_consensus_summits_lo.bed"

  # Run halLiftover, either with Docker or directly
  echo "Lifting over consensus summits for $sp..." | tee -a "$hal_liftover_log"
  if [[ "$use_docker" == true ]]; then
    docker run --rm -v "$base_folder:$base_folder" -w "$base_folder" "$docker_image" halLiftover "$HAL_FILE" "$ANCESTRAL_GENOME" "$consensus_input_file" "${sp^}" "$consensus_output_file"  >> "$hal_liftover_log" 2>&1
  else
...
hal_liftover_log="${current_folder}/stepC_halLiftover.log"
for source_species in "${species[@]}"; do
  for target_species in "${species[@]}"; do
    ...
    input_summits_file="${current_folder}/${source_species}_spec_summits.bed"
    output_summits_file="${current_folder}/${source_species}_on_${target_species}_spec_summits_lo.bed"
    ...
    if [[ "$use_docker" == true ]]; then
      docker run --rm -v "$base_folder:$base_folder" -w "$base_folder" "$docker_image" halLiftover "$HAL_FILE" "${source_species^}" "$input_summits_file" "${target_species^}" "$output_summits_file" >> "$hal_liftover_log" 2>&1
    else
    ...
    input_peaks_file="${current_folder}/${source_species}_spec_peaks.bed"
    output_peaks_file="${current_folder}/${source_species}_on_${target_species}_spec_peaks_lo.bed"
    ...
    if [[ "$use_docker" == true ]]; then
      docker run --rm -v "$base_folder:$base_folder" -w "$base_folder" "$docker_image" halLiftover "$HAL_FILE" "${source_species^}" "$input_peaks_file" "${target_species^}" "$output_peaks_file" >> "$hal_liftover_log" 2>&1
    else
...
Rscript "${base_folder}/CrossPeak_Step7.R" > "${current_folder}/step7.log" 2>&1
if [[ $? -ne 0 ]]; then
  echo "Error: Step 7 failed. Check ${current_folder}/step7.log for details."
  exit 1
fi

# Step D: Reciprocal liftover for round 2 peaks
echo "Running Step D: Reciprocal halLiftover for round 2 peaks..."
hal_liftover_log="${current_folder}/stepD_halLiftover.log"
for source_species in "${species[@]}"; do
  for target_species in "${species[@]}"; do
    # Skip if source and target species are the same
    if [[ "$source_species" == "$target_species" ]]; then
    ...
    # Generate file names
    input_file="${current_folder}/${target_species}_on_${source_species}_rd2_con_peaks.bed"
    output_file="${current_folder}/${target_species}_from_${source_species}_rd2_con_peaks_lo.bed"
    ...
    if [[ "$use_docker" == true ]]; then
      docker run --rm -v "$base_folder:$base_folder" -w "$base_folder" "$docker_image" halLiftover "$HAL_FILE" "${source_species^}" "$input_file" "${target_species^}" "$output_file" >> "$hal_liftover_log" 2>&1
    else
...
if [[ "$cleanup" == true ]]; then
  ...
  log_files=$(find "$current_folder" -maxdepth 1 -type f -name "*.log")
  ...
  for file in $(find "$current_folder" -maxdepth 1 -type f); do
  ...
  echo "Cleanup flag is set to false. Intermediate files are retained in ${current_folder}."
fi

# Final message
echo "CrossPeak completed successfully. Logs are in ${current_folder} and output files are in ${output_folder}."
